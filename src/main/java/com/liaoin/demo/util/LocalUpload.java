...
import com.liaoin.demo.entity.FileInfo;
...
public class LocalUpload {
     ...
     */
    public static synchronized R<FileInfo> fragmentFileUploader(UploadFileParam param, String confFilePath,
     ...
     * @throws Exception 抛出自定义Exception异常
     */
    public static R<JSONArray> checkFileMd5(String fileMd5,
                                                 String fileName,
                                                 String confFilePath,
        ...
        if (confFile.exists() && isTmpFileEmpty) {
            ...
            return new R(HttpStatus.PARTIAL_CONTENT.value(), "文件已经上传了一部分",
        ...
        if (isFileEmpty && confFile.exists()) {
            return new R(HttpStatus.OK.value(), "文件已上传成功");
        }
        return new R(HttpStatus.NOT_FOUND.value(), "文件不存在");
    }
}
